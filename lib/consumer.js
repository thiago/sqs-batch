...
class Consumer extends EventEmitter {
  ...
  _internalMessageProcessor (messages) {
    ...
    this.emit('message:received', messages)
  ...
  _deleteMessageBatch (messages, cb) {
    ...
    while(messages.length) {
      ...
      this.sqs.deleteMessageBatch(params, (err, data) => {
        if (err) {
          return cb(new Errors.SQSError(`SQS delete message batch failed with error: ${err.message}`))
        }
        if (data.Failed.length) {
          return cb(new Errors.SQSError(`SQS delete message batch failed with errors: ${data.Failed}`))
        }
        Debug('Batch deleted!')
        cb()
      })
  ...
  _handleErrors (error) {
    switch (error.constructor) {
      ...
      case Errors.ProcessingError:
        ...
        return this.emit('processing:error', error)
    }
  }
  ...
  deleteMessage (message) {
    if (!message) {
      return this._handleErrors(new Errors.ConsumerDeleteError('No message supplied to be deleted'))
    }

    if (_.isError(message)) {
      return this._handleErrors(new Errors.ProcessingError(message))
    }

    if (!Array.isArray(message) || message.length === 1) {
      this._deleteMessageOne(message[0] || message, err => {
        if (err) return this._handleErrors(err)
        this._messageProcessed(message)
      })
    } else {
      this._deleteMessageBatch(message, err => {
        if (err) return this._handleErrors(err)
        this._messageProcessed(message)
      })
    }
